		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			...
			&& (oplus_chg_get_curr_time_ms(&cur_chg_time) < 10000))
			/*&& oplus_vooc_get_fastchg_started() != true)*/ {
				val->intval = 5000;
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((get_eng_version() == HIGH_TEMP_AGING
			|| get_eng_version() == PTCRB
			|| get_eng_version() == AGING) && chip->charger_type != POWER_SUPPLY_TYPE_USB) {
...
static void oplus_chg_other_thing(struct oplus_chg_chip *chip)
{
	...
	power_supply_changed(g_charger_chip->usb_psy);
#ifndef CONFIG_OPLUS_CHARGER_MTK
#ifndef WPC_NEW_INTERFACE
...
static int oplus_chg_get_voocphy_support_display_vooc(void)
{
	if (!g_charger_chip) {
		return false;
	} else {
...
void oplus_chg_vooc_timeout_callback(bool vbus_rising)
{
	if (!g_charger_chip) {
		...
		if(g_charger_chip->chg_ops->vooc_timeout_callback) {
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		if (oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY && g_charger_chip->chg_ops->get_subboard_temp) {
