...
bool oplus_chg_check_pd_svooc_adapater(void)
{
	...
	if (!g_voocphy_chip) {
		voocphy_err("!g_voocphy_chip is null return\n");
		return true;
	}
	if (!g_oplus_chip) {
     		voocphy_err("g_oplus_chip is null return\n");
		return true;
	}

	if (OPLUS_PDSVOOC_ADATER_ID == g_voocphy_chip->adapter_model_ver && ADAPTER_SVOOC == g_voocphy_chip->adapter_type) {
		pdsvooc = true;
		g_oplus_chip->pd_svooc = pdsvooc;
	}
    	voocphy_info("oplus_chg_check_pd_svooc_adapater : curent adapter %s pdsvooc \n", pdsvooc ? "is" : "is not");
	voocphy_info("oplus_chg_check_pd_svooc_adapater : adapter_model_ver:0x%x, adapter_type:0x%x\n", g_voocphy_chip->adapter_model_ver, g_voocphy_chip->adapter_type);
...
int oplus_voocphy_handle_tell_model_process_cmd(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->adapter_model_count == 2) {
		...
		if ((chip->adapter_model_ver == 0x13 || chip->adapter_model_ver == 0x12) && chip->adapter_type != ADAPTER_SVOOC) {
...
extern oplus_chg_get_chg_type(void);
#define MAX_IGNORE 6
...
irqreturn_t oplus_voocphy_interrupt_handler(struct oplus_voocphy_manager *chip)
{
	...
	if (POWER_SUPPLY_TYPE_USB_DCP != oplus_chg_get_chg_type()) {
		chg_err("oplus_chg_get_chg_type is not DCP\n");
		return IRQ_HANDLED;
	}
	...
	if (chip->vooc_flag & RXDATA_DONE_FLAG_MASK) {
		...
		if (chip->irq_rcvok_num <= MAX_IGNORE &&
		    chip->start_vaild_frame == false) {
		...
		if (chip->start_vaild_frame == false && chip->irq_rcvok_num > MAX_IGNORE) {
			chip->fastchg_allow = false;
			chip->fastchg_start = false;
			chip->fastchg_to_warm = false;
			chip->fastchg_dummy_start = false;
...
void oplus_voocphy_switch_fast_chg(void)
{
	...
	if (oplus_voocphy_get_fastchg_to_normal()
		...
		if (oplus_voocphy_get_real_fastchg_allow() == false
			...
			if( oplus_voocphy_get_fastchg_dummy_start() && (g_voocphy_chip->pd_non_support)){
				voocphy_info("pd_non_support sleep\n");
				msleep(2000);
			}
		}
	}
...
int oplus_voocphy_parse_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	chip->pd_non_support = of_property_read_bool(node,
	                               "qcom,pd_non_support");
	voocphy_info("pd_non_support = %d\n", chip->pd_non_support);
