...
{
	return (strncmp(oplus_chg_ops_name_get(), "ext-mp2650", 64) == 0);
}
...
void oplus_otg_enable_by_buckboost(void)
{
	...
	if (g_oplus_chip->chg_ops->get_otg_enable && g_oplus_chip->chg_ops->get_otg_enable()) {
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	//chg_err("oplus_ccdetect_work:level:%d\n", level);
	if (level != 1) {
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->dual_charger_support) {
		if ((chip->charger_volt > 7500 && chip->soc >= 90 && chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr)
		...
		} else if (chip->charger_volt < 6500
				&& chip->soc < 90
